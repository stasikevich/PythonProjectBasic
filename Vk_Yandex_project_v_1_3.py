...
class VK:
    ...
    def __init__(self, VK_USER_ID, VK_USER_TOKEN, album_id="profile"):
    ...
    def get_photo_data(self, offset="0", count="5"):
        ...
        api = requests.get('https://api.vk.com/method/photos.get',
                           params={'owner_id': self.VK_USER_ID, 'access_token': self.VK_USER_TOKEN,
                                   'album_id': self.album_id, 'extended': "1", "count": self.count, 'offset': self.offset, "photo_sizes": '1', 'v': '5.131'})
    ...
    def get_photo(self, photos_folder):
        ...
        while index <= count_photo:
            ...
            index += count
        pprint(len(photos))
        pprint(f'Фотографии юзера {self.VK_USER_ID} были загружены в папку {photos_folder} список фотографий и их '
               f'размер  {self.photo_json}')
...
def menu():
    folder_name = input(f'What is your folder name: ')

    photos_folder = create_folder(folder_name)

    VK_USER_TOKEN = input(f'Enter your VK token number: ')

    VK_USER_ID = input(f'Enter VK ID you want to download photos: ')
    yandex_token = input(f'What is your yandex token number: ')

    ya_folder = input(f'Folder name on Yandex Disk :')
